...
```{r}
write.csv(clinic, "/Users/kojiro/Desktop/qbio490/qbio_490_KainingFeng/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/kojiro/Desktop/qbio490/qbio_490_KainingFeng/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 3.1}
# write code here
summary.data.frame(clinic)
nrow(clinic)
ncol(clinic)
clinic$gender
clinic[5,"days_to_birth"]/365
clinic[1,"days_to_birth"]/365 - clinic[1,"days_to_last_followup"]/365 
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic$days_to_death,clinic$days_to_birth,main = "days to death vs days to birth", xlab="days to death",ylab = "days to birth",col=2)
```
...
```{r Exercise 4.2}
# create plot here
boxplot(clinic$age_at_initial_pathologic_diagnosis~clinic$race_list,data=clinic, main="age at initial pathologic diagnosis vs race list",xlab="age at initial pathologic diagnosis",cex.axis=1)
```
