...
Use the dim() function to find the dimensions of the colData, rowRanges, and unstranded dataframes. Record them below:
  ...
  ~ rowRanges: genes: 60660    10
  ~ unstranded:counts: 60660  1231
...
Run the following code and fill out a description of the rows/columns of our data frames.You can use this as a cheat sheet moving forward.
 ~ colData
        - Rows: patients
        - Columns: patient info
  ~ rowRanges
        - Rows: genes
        - Columns: gene info
  ~ unstranded (in assays)
        - Rows: genes
        - Columns: gene counts for a particular patient
...
```{r Exercise 2.6}
write.csv(rna_clinical, "/Users/kojiro/desktop/qbio490/qbio_490_KainingFeng/analysis_data/brca_rna_clincial_data.csv", row.names = TRUE)
```
...
```{r Exercise 3.4}
write.csv(rna_genes, "/Users/kojiro/brca_rna_gene_data.csv", row.names = FALSE)
```
...
```{r Exercise 4.4}
write.csv(rna_counts, "/Users/kojiro/desktop/qbio490/qbio_490_KainingFeng/analysis_data/brca_rna_count_data.csv", row.names = TRUE)
```
...
plot(TP53_counts,
     MUC16_counts,
     xlab = "TP53 RNA Counts",
     ylab = "MUC16 RNA Counts",
     main = "MUC16 RNA Counts vs. TP53 RNA Counts in Cancerous Tissues"
)
```
...
There doesn't seem to be an obvious pattern. The MUC16 gene constantly have a low expression. In this case, no curves or slopes are observed. Future sesearch may still need to figure out their relationships. 
----
...
The range of MUC16 gene is much broader than the range of TP53 gene, meaning is more various and more unstable. In this case, the more stable TP53 gene with less outliers are more worth studying and is more likely have relationship with cancer. 
----
...
```{r Exercise 7.3}
par(mar=c(1,1,1,1))
boxplot(TP53_counts ~ five_yr_death_and_followup, # think about what data (gene counts or the 5 yr survival column) should be on the x vs y axes
        xlab = "Did the patient survived past 5 years?",
        ylab = "TP53 RNA Counts")
dev.off()
```
...
It appears that patients who survived past five years have similar expression of TP53 and MUC16.
It's hard to make solid conclusion based on this plot, and P-value needs to be calculated in order
to quantitatively confirm the observation. Future research is still needed to come up with a conclusion. 
Based on my research MUC16 mutation may be associated with superior response to ICIs in patients with solid tumors.
https://jamanetwork.com/journals/jamanetworkopen/fullarticle/2769828
----
...
colnames(geneABCD_counts) <- c("TP53 Counts", "MUC16 Counts", "MKI67 Counts", "PTGS2 Counts")
...
cols[is.na(rna_clinical$age_category)] <- "white"
...
```{r Exercise 8.1}
# save files here
write.csv(geneABCD_counts, "/Users/kojiro/desktop/qbio490/qbio_490_KainingFeng/analysis_data/ABCD.csv",row.names = TRUE)
```
